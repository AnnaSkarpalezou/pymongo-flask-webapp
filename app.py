...
db = connection[config['MONGO_DBNAME']]["main"] # store a reference to the database
db1 = connection[config['MONGO_DBNAME']]["archive"]
...
def archive():
    ...
    Displays some information for the user with links to other pages.
    """
    docs = db1.exampleapp.find({}) # sort in descending order of created_at timestamp
    return render_template('archive.html', docs=docs) # render the read template
...
#     Displays a form users can fill out to create a new document.
#     """
#     return render_template('rate.html') # render the create template
...
def rate(mongoid):
    ...
    return render_template('rate.html', mongoid=mongoid, doc=doc) # render the edit template


@app.route('/rate/<mongoid>', methods=['POST'])
def rate_post(mongoid):
    """
    ...
    db1.exampleapp.insert_one(doc) # insert a new document
    return redirect(url_for('archive')) # tell the web browser to make a request for the /read route.
...
def delete(mongoid):
    ...
    return render_template('rate.html', mongoid=mongoid, doc=doc)
